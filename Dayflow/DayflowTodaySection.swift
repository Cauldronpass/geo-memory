...
enum DayflowMeetingActions {
    // `noteStem` moved to `DayflowAgendaMatch` (Session 82, D244) — see the
    // note on it there. This enum stays in Dayflow because `openMeetingNote`
    // ends in `DayflowQuickFindRouter`, which is Dayflow-only routing.
