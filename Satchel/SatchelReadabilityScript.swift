// SatchelReadabilityScript.swift
// Mozilla Readability 0.6.0, vendored as a Swift string rather than a bundled
// resource. Apache License 2.0; the upstream notice is reproduced below and at
// the head of the script itself.
//
// **Why a Swift file and not `Readability.js` in the folder.** A buildable
// folder assigns files to build phases by type, and whether a `.js` lands in
// Copy Bundle Resources is a thing you have to open Xcode and look at. If it
// lands nowhere the app does not fail: `Bundle.main.url(forResource:)` returns
// nil, the fetch reports itself unavailable, and every saved link quietly never
// gets read. A silent nothing that looks like a working app is the failure
// shape this project keeps writing warnings about, and here it was also a
// manual verification step on David's side for no benefit. A string in a source
// file is compiled in or the build fails. There is no third outcome.
//
// Cost, stated: updating Readability means regenerating this file from the npm
// tarball rather than dropping a new `.js` in. The regeneration is three lines
// and the version moves about twice a year.
//
// Source: npm `@mozilla/readability@0.6.0`, file `Readability.js`,
...
enum SatchelReadabilityScript {
    /// A RAW string (`#"""`), so nothing in 2800 lines of JavaScript is read as
    /// Swift interpolation. Backslashes in its regular expressions stay
    /// backslashes.
    static let source = #"""
/*
 * Copyright (c) 2010 Arc90 Inc
