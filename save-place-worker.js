...
async function handleSave(data, env, ctx) {
  ...
  if (data.lat  != null)         props['Latitude']         = { number: Number(data.lat) };
  if (data.lng  != null)         props['Longitude']        = { number: Number(data.lng) };
  ...
  if (data.rating_external != null) props['Rating External'] = { number: Number(data.rating_external) };
  if (data.phone)                props['Phone']             = { phone_number: data.phone };
  if (data.website)              props['Website']           = { url: data.website };
  if (data.price_level != null)  props['Price Level']       = { number: Number(data.price_level) };
